...
input_path = r"CelebATrain/"
validation_path = r"CelebAValidation/"
test_path = r"CelebATest/"

base_dataset_dir = os.path.expanduser("/home/nbayat5/") + "Image-Super-Resolution/"

output_path = base_dataset_dir + "train_images/train/"
validation_output_path = base_dataset_dir + r"train_images/validation/"
test_output_path= base_dataset_dir + r"test_images/"
...
def transform_images(directory, output_directory, scaling_factor=4, max_nb_images=-1, true_upscale=False):
    index = 1
    ...
    if not target_shape:
        if small_train_images:
            if K.image_dim_ordering() == "th":
                image_shape = (channels, 16 * scale_factor * _image_scale_multiplier,
                                 16 * scale_factor * _image_scale_multiplier)
                y_image_shape = (channels, 16 * scale_factor * _image_scale_multiplier,
                                 16 * scale_factor * _image_scale_multiplier)
            else:
                image_shape = (16 * scale_factor * _image_scale_multiplier,
...
if __name__ == "__main__":
    ...
    # transform_images(input_path, output_path, scaling_factor=4, max_nb_images=-1, true_upscale=True)
    # transform_images(validation_path, validation_output_path, scaling_factor=4, max_nb_images=-1, true_upscale=True)
    transform_images(test_path, test_output_path, scaling_factor=4, max_nb_images=-1, true_upscale=True)
