...
def calculate_fid(model, images1, images2):
    ...
    print("Images shape after reshape is: ", images2.shape)
    images1 = images1.astype('float32')
    images2 = images2.astype('float32')
    # resize images
    images1 = scale_images(images1, (75, 75, 3))
    images2 = scale_images(images2, (75, 75, 3))
    print("Images shape after scale is: ", images2.shape)
    images1 = preprocess_input(images1)
    images2 = preprocess_input(images2)
    print("Images shape after pre process is: ", images2.shape)
...
class BaseSuperResolutionModel(object):
    ...
    def upscale(self, img_path, save_intermediate=False, return_image=False, suffix="scaled",
        ...
        root = path[0].split("/")
        idx = len(root)
        filename = root[idx-1] + "_" + suffix + "(%dx)" % (self.scale_factor) + path[1]
        filename = "val_predict/" + filename
        # Read image
...
def _evaluate(sr_model : BaseSuperResolutionModel, Test_dir, scale_pred=False):
    """
    ...
    image_fns = [name for name in os.listdir(Test_dir)]
    nb_images = len(image_fns)
    print("Validating %d images from path %s" % (nb_images, Test_dir+"X/"))
    ...
    ##########Start Processing each image in test set#############
    counter = 0
    for impath in os.listdir(Test_dir+"X/"):

        t1 = time.time()

        # Input image
        y = img_utils.imread(Test_dir+"y/" + impath, mode='RGB') #Read an image from a file as an array
        x = img_utils.imread(Test_dir+"X/" + impath, mode='RGB')
        ...
        if sr_model.type_scale_type == "tanh":
            ...
            x_width = 64 if not sr_model.type_true_upscaling else 16
            x_height = 64 if not sr_model.type_true_upscaling else 16
            x = img_utils.imresize(x, (x_width, x_width),
                                     interp='bicubic')
            if not sr_model.type_true_upscaling:
                x = img_utils.imresize(x, (x_width, x_height), interp='bicubic')
        ...
        ssim_val = compare_ssim(y[0], np.clip(y_pred, 0, 255) / 255,  multichannel=True)
        total_psnr.append(psnr_val)
        total_ssim.append(ssim_val)
        images1.append(y[0])
        images2.append(np.clip(y_pred, 0, 255) / 255)
    ...
    print("Average SSIM value of validation images = %00.4f \n" % (np.mean(total_ssim)))
    fid_score(images1, images2)
